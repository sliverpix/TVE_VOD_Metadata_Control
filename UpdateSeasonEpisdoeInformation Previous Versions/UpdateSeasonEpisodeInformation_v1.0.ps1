...
# to CATEGORY, SUBSCRIPTION_TYPE and SUBSCRIPTION_BRAND. ONLY HLS title
# types are checked. XML files are saved to ORIGINAL and MODIFIED 
# folders to maintain the integrity of our library. We target HLS 
# formats in both HD and SD and set values based on PROVIDER value.
#
...
# Version:  1.0
# History:  09-20-17	- Initial release
#
####################################################################
#
# performance ideas
...
function Get-SeasonEpisode($stringToCheck) {
	...
	$rgx_type2 = [regex] '\sS\d{1,2}:\s\d{1,3}'	# very loose but looks for the space between season and episode
												# (example: S9: 005 = season 9 episode 5)
	
	$rgx_type3 = [regex] '\d{2,2}_\d{1,3}'		# ##_### (example: (Vice News) 04_014 = season 4 episode 14)
		
	$rgx_type4 = [regex] '\s\d{2,3}'		    # ### / ### (format of SEE - season: Episode# Episode#)
    ...
    #Write-Debug("Type Cast and Trimmed: $($cleanString)")
	$s1 = ""
	$e1 = ""
	Switch -regex ($cleanString)
		{			
			...
			$rgx_type4 {
                            ...
                            Write-Debug($matches.Values)
						    Write-Host("I loosely matched TYPE 4 pattern. No change was made.") -ForegroundColor Yellow
							$script:numWarn++
						    Write-Log($xml_filename, "[WARN] TYPE4 match detected. This could be a flase positive. No Change was made to the meta file!")
...
Foreach ($line in $contents)
{
    ...
    Foreach ($value in $DataSet.Tables[0])
    {
		...
		if ($app_contentType.value -eq "TVS"){
			...
			# check for TYPE 4 match.. very loose regex. make no change but inform
			# operator to check meta to collect pattern for use later.
			# else set EPISODE_ID and SEASON_ID
			if ($exTypeMatch -eq 4){
				$e_message = "TYPE 4 match is very loose. Check META and log. No changes made to meta"
				...
				Write-Log($xml_filename, "[WARN] If this is correct, please change SEASON/SEASON_ID and EPISODE_ID in the meta.")
				Break;
			} else {
			# match or set season and episode numbers
			# do we nedd to check that the noed/element is present? and build it?
				$app_EpisodeID = $exEpisode
				$app_Season = $exSeason
				$app_SeasonID = $exSeason
