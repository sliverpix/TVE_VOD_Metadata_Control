...
#			08-01-2018	- (1.7.1) reverted to a -Replace switch in the cleanup() functions to address the wierd returns from the various "(HD)" 
#							matches we were getting with the .Replace method.
#						- (bug) case where "&amp;" is being passed as "&" within powershell variables but not being passed to the XML.
#			08-08-2018	- (1.8) Add secondary logic to extrapolate SEASON/EPISODE number. If TITLE_BRIEF and EPISODE_NAME fails to match, 
#							check TITLE for a match to our patterns.
#						- Cleanup logging and debug statements.
#						- fixed mispellings/bugs (uncaught somehow)
#			08-21-2018	- (1.9) Added new branch of logic to help find and set Episode Name, Season and Episode number by PROVIDER.
#							user input is taken at the start of the script, to either set/change these values by either CATEGORY or by PROVIDER in 
#							our CSV files. Thus, the user can choose which logic to use. The rest of the logic remains the same.
#						- Get-SeasonEpisodebyProvider() will check for EPISODE_NAME node and build it, if it is missing. It will take CATEGORY_DISPLAY
#							value and split it to an array. then take each value in the array and look for a match in the csv_Providers.inc file.
#							if it finds a match it will set the EPISODE_NAME to the Title_Brief value, set the SEASON and EPISODE_NUMBER to '0'
#						- All other logic is the same as previously built/used. Thus choosing "By Category" will follow the original logic chain.
#			08-28-2018	- (1.9.1) adding value check to Category_Display and Category nodes. If we find MOV values from provider CSV, change
#							them to TVS values.
#			12-12-2018	- (1.10) Powershell version changed to 4.0.1.1 and we lost access to Rows.Count
#							ADD Dataset.Tables.Rows | Measure-Object to get number of rows returned from our SQL query. If no ROWS returned 
#							we break out since there is no data for us work with. The incident is also logged and output set to the user.
...
function Get-SeasonEpisodebyProvider{
	...
	if(!($app_EpisodeName)){
		...
		$app_elem.SetAttribute("Name","Epipsode_Name")
...
Foreach ($line in $contents){
    ...
    Foreach ($value in $DataSet.Tables[0])
    {
		...
		#[Regex]::Escape($value.strTitle)
		$cleanuptitle = $value.strTitle.toString()
		$cleanuptitle = $cleanuptitle.Replace(":","")
		$cleanuptitle = $cleanuptitle.Replace(".","")
		$cleanuptitle = $cleanuptitle.Replace("/","")
		$cleanuptitle = $cleanuptitle.Replace("&amp;","and")
		$cleanuptitle = $cleanuptitle.Replace(" ","_")
		$xml_filename = ($cleanuptitle + "_" + $alt_code + "_" + $value.strscreenFormat + ".xml")
