...
#			05-08-18	- add isReview flag to fix SD saving bug. Where some SD meta was getting skipped due
#							broken season/episode extrapolation failure.
...
Foreach ($line in $contents)
{
    ...
    Foreach ($value in $DataSet.Tables[0])
    {
		...
		if($isReview -eq 0){
			...
			Write-Log($xml_filename, "[$($logLevel)] $($e_message)")
			Write-Log($xml_filename, "[$($logLevel)] String sent to function: $($SeasonEpisodeSTR)")
			Write-Log($xml_filename, "[$($logLevel)] SEASON String we matched: $($exSeason)")
			Write-Log($xml_filename, "[$($logLevel)] EPISODE String we matched: $($exEpisode)")
			
			# match or set season and episode numbers
			Write-Debug ("SEASON is $($app_Season.value) ==>> changing to: $($exSeason)")
			Write-Debug ("EPISODE is $($app_EpisodeNum.value) ==>> changing to: $($exEpisode)")
