...
#						- Add Directory sorting for meta that changes some nodes but does not
#							match our REGEX. (makes it easier on our folks making manual changes)
#
####################################################################
#
# performance ideas
	...
	Switch -regex ($cleanString)
		{			
            ...
            $rgx_type7 {
							...
							$script:numWarn++
						    Write-Log($xml_filename, "[WARN] TYPE 7 match detected. This could be a false positive.")
			...
			$rgx_type6 {
							...
							$script:numWarn++
						    Write-Log($xml_filename, "[WARN] TYPE 6 match detected. This could be a false positive.")
						    Write-Log($xml_filename, "[WARN] String: $($cleanString)")
...
function Summarize(){
	...
	Write-Host $numRev -ForegroundColor Cyan
...
Foreach ($line in $contents)
{
    ...
    Foreach ($value in $DataSet.Tables[0])
    {
        ...
        if (!($app_SeriesName)){
			...
			write-log ($xml_filename, "[WARN] Finished building Series_Name node. It is empty currently.")
			...
			Write-Log ($xml_filename,"[INFO] SERIES_NAME found: $($app_SeriesName.value). Cleaning it up...")
			$seriesNameClean = cleanUp($app_SeriesName.Value)
			$app_SeriesName.value = $seriesNameClean
			Write-Debug ("Finished!")
			Write-Debug ($app_SeriesName.value)
			Write-Log ($xml_filename, "[INFO] Finished cleaning: $($app_SeriesName.value)")
		}
		...
		if (!($app_SeriesID)){
			...
			write-log ($xml_filename, "[WARN] Finished building Series_ID node. It is empty currently.")
			Write-Host ("Fixed. Check log.") -ForegroundColor Green
		} else{
			# the element node exists so clean it up
			Write-Debug ("Found SERIES_ID. Cleaning it up!")
			Write-Debug ($app_SeriesID.value)
			Write-Log ($xml_filename,"[INFO] SERIES_ID found: $($app_SeriesID.value). Cleaning it up...")
			$seriesIDClean = cleanUp($app_SeriesID.Value)
			$app_SeriesID.value = $seriesIDClean
			Write-Debug ("Finished!")
			Write-Debug ($app_SeriesID.value)
			Write-Log ($xml_filename, "[INFO] Finished cleaning: $($app_SeriesID.value)")
		}
		
		# SERIES_DESCRIPTION NODE
		if (!($app_SeriesDesc)){
			$e_message = "Series_Description node is MISSING !! Building node..."
			$numWarn++
			write-log ($xml_filename, "[WARN] $($e_message)")
			Write-Host ($e_message) -ForegroundColor yellow
			...
			$app_elem = $content.CreateElement("App_Data")
			$app_elem.SetAttribute("App","$($AMS_product)")
			$app_elem.SetAttribute("Name","Series_Description")
			$app_elem.SetAttribute("Value","")	
			$app_SeriesDesc = $content.ADI.Asset.Metadata.AppendChild($app_elem)
			write-log ($xml_filename, "[WARN] Finished building Series_Description node. It is empty currently.")
			Write-Host ("Fixed. Check log.") -ForegroundColor Green
		} else{
			# the element node exists so clean it up
			Write-Debug ("Found SERIES_DESCRIPTION. Cleaning it up!")
			Write-Debug ($app_SeriesDesc.value)
			Write-Log ($xml_filename,"[INFO] SERIES_DESCRIPTION found: $($app_SeriesDesc.value). Cleaning it up...")
			$seriesDescClean = cleanUp($app_SeriesDesc.Value)
			$app_SeriesDesc.value = $seriesDescClean
			Write-Debug ("Finished!")
			Write-Debug ($app_SeriesDesc.value)
			Write-Log ($xml_filename, "[INFO] Finished cleaning: $($app_SeriesDesc.value)")
		}
		...
		if ($app_contentType.value -ne "TVS"){
			...
			Write-Log($xml_filename, ("[WARN] $($e_message). Changing $($app_contentTypeWas) to $($app_contentType.value)"))
		}
		...
		if (IsNull($exTypeMatch) -or IsNull($exSeason) -or IsNull($exEpisode)){
			...
			Write-Log($xml_filename,"[INFO] Saving changes made so far in \REVIEW\ .")
