...
function cleanUp($dirtyString, [switch]$isSeries){
	...
	if (!($isSeries)){
		...
		Write-Debug ("CleanUP returning STRINGL: $($cleanString)")
		return (Get-Culture).textinfo.totitlecase($cleanString.tolower())
	}
}
...
Foreach ($line in $contents)
{
    ...
    Foreach ($value in $DataSet.Tables[0])
    {
		...
		foreach($orgfilename in gci $originalD -Name -File)
		{
			write-debug("Checking for duplicate filename...")
			if ($orgfilename -contains $xml_filename)
			{
				write-debug("We Already processed $($xml_filename). Skipping!")
				write-log $xml_filename "W" "$($xml_filename) was already processed. Skipping!"
			} else {
				write-debug("No duplicate filename found for $($xml_filename). Saving ORIGINAL.")
				$content.Save($originalD + "\" + $xml_filename)
				$numOrig++
			}
		}
		...
		<#
		if ($value.strScreenFormat -like "*_HD*") {
			Write-Host("... HD meta")
		} else {
			Write-Host("... HD meta NOT found!") -ForegroundColor Red
			Write-Log $xml_filename "E" "No HD XML meta was found. Is this it he right Asset ID?"
		}
		if ($value.strScreenFormat -like "*_SD*") {
			Write-Host("... SD meta")
		} else {
			Write-Host("... SD meta NOT found!") -ForegroundColor Red
			Write-Log $xml_filename "E" "No SD XML meta was found. Is this it he right Asset ID?"
		}
		#>
		...
		if($isReview -eq 0){
			...
			Write-Log $xml_filename $llevel "SEASON String we matched: $($exSeason)"
