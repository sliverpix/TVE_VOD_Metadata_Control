...
#						- (bug) case where "&amp;" is being passed as "&" within powershell variables but no what is being passed to the XML.
#							
#
####################################################################
#
...
(
	...
	# 1 = found a match
	$matchfound = 0
	write-debug ("[Find_SeriesName.categorycheck Matchfound set: $($matchfound)")
    ...
    if($matchfound -eq 0){
        Write-debug("[Find_SeriesName] Didnt find a match for $($stringtocheck)")
    }
	...
	if($matchfound -eq 0){
		write-Host("[Find_SeriesName] No match was found. Returning: $($stringtocheck)") -ForegroundColor Red
		RETURN [string]$stringtocheck
	} else {
		Write-Host("[Find_SeriesName] Found Match. Returning: $($returnString)")
    ...
    Foreach ($value in $DataSet.Tables[0])
    {
		...
		if (IsNull($app_TitleBrief.value)){
			...
			if (IsNull($app_EpisodeName.value)){
                ...
                #save uncleaned string for use later so we dont break our functions checks
