...
# saves on memeory and outputs minimal text to the screen
#
...
(
	...
	Foreach ($item in $seriesDict){
		if ($stringtocheck -eq $item."BLOB_NAME"){
			...
			$matchfound = 1
			write-debug ("[Find_SeriesName.categorycheck Matchfound changed: $($matchfound)")
		}
	}
...
Foreach ($line in $contents)
{
    ...
    Foreach ($value in $DataSet.Tables[0])
    {
		...
		if (!($app_SeriesID) -or (IsNull $app_SeriesID.value)){
			...
			$app_SeriesID.value = $seriesIDClean.toString()
